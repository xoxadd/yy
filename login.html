...
if (isset($_POST['add_student'])) {
    ...
    if (!$lrn) {
        ...
        if ($result->num_rows > 0) {
            ...
            // Insert student with default password 'student123' and is_active as TRUE (active)
...
// Fetch vote statistics for dashboard (calculate percentage of students who voted)
$total_students = $conn->query("SELECT COUNT(*) FROM users WHERE role='student'")->fetch_row()[0];
$voted_students = $conn->query("SELECT COUNT(DISTINCT voter_id) FROM votes")->fetch_row()[0];
$not_voted_students = $total_students - $voted_students;

$voted_percentage = $total_students > 0 ? ($voted_students / $total_students) * 100 : 0;
...
<div class="container">
  ...
  <div class="main">
    ...
    <div id="dashboard" class="section active">
      ...
      <div class="cards">
        <div class="card">
          <h1 id="totalStudents"><?php echo $total_students; ?></h1>
          <p>Total Students</p>
        </div>
        <div class="card">
          <h1 id="activeDisplay"><?php echo $voted_students; ?></h1>
          <p>Voted Students</p>
        </div>
      </div>
            ...
            <tr>
              ...
              <td>
                <form method="POST" style="display:inline;">
                  <button type="submit" name="delete_student" value="<?php echo $student['id']; ?>" style="background:#ff7676;color:white;border:none;border-radius:8px;padding:6px 12px;">Delete</button>
                </form>
              </td>
            </tr>
          <?php endwhile; ?>
...
// Chart for Dashboard - Voting Stats
...
const studentChart = new Chart(ctx, {
  ...
  data: {
    ...
    datasets: [{
      label: "Voting Status",
      data: [<?php echo $voted_percentage; ?>, <?php echo $not_voted_percentage; ?>],
